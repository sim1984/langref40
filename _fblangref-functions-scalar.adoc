...
[NOTE]
====
Ещё раз обратите внимание на то, что пространства имён и имена переменных чувствительны к регистру символов. Они должны быть не пустыми строками, и заключены в кавычки!
====
...
Сравнений значений `DECFLOAT` происходит в следующем порядке:
...
Функция возвращает результат типа `BIGINT`, если в качестве первого аргумента явно не передано число типа `INT128`. Таким образом, следующие выражения дадут разные результаты.
...
[[fblangref-scalarfuncs-system-privilege]]
...
Функция `RDB$SYSTEM_PRIVILEGE` возвращает `TRUE`, если системная привилегия используется текущим соединением, в противном случае -- `FALSE`. +
Список системных привилегий см. в <<fblangref-security-roles-create,CREATE ROLE>>.
