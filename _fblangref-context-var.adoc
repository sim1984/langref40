...
Переменная `CURRENT_CONNECTION` хранит уникальный идентификатор текущего соединения. Значение переменной хранится в странице заголовка базы и сбрасывается после восстановления базы данных с помощью инструмента `gbak`. Переменная увеличивается на единицу при каждом последующем соединении с базой данных (соединения также могут быть внутренними вызванными самим ядром). Следовательно, переменная показывает количество подключений произошедших к базе после её восстановления (или после её создания).
...
|precision
|Точность. Значение по умолчанию 0. +
...
|precision
|Точность. Значение по умолчанию 3. +
Не поддерживается в ESQL.
|===
...
операторов `EXECUTE STATEMENT` или `EXECUTE PROCEDURE`. Для оператора `MERGE` переменная `ROW_COUNT` будет содержать 0 или 1, даже если было затронуто более записей
====
