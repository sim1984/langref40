...
Если заголовка пакета с именем [replaceable]``package_name`` не существует, то будет выдана соответствующая ошибка.
...
* Значения по умолчанию для параметров процедур не могут быть переопределены (которые указываются в [replaceable]``<package_item>``). Это означает, что они могут быть в [replaceable]``<package_body_item>`` только для частных процедур, которые не были объявлены.
