...
В простой позиционной форме (с `FROM`) эта функция возвращает подстроку, начинающуюся с позиции символа _startpos_ (первая позиция равна 1). Без аргумента `FOR` он возвращает все оставшиеся символы в строке.
...
<<fblangref-datatypes-datetime-operation,"Операции, использующие значения даты и времени">>.
