...
0 -- BY_VALUE -- по значению; +
1 -- BY_REFERENCE -- по ссылке; +
2 -- BY_VMS_DESCRIPTOR -- через дескриптор; +
3 -- BY_ISC_DESCRIPTOR -- через дескриптор BLOB; +
4 -- BY_SCALAR_ARRAY_DESCRIPTOR -- через дескриптор массива; +
5 -- BY_REFERENCE_WITH_NULL -- по ссылке с NULL признаком.
...
|SMALLINT
|Механизм передачи параметра (не используется в Legacy функциях):

0 -- NORMAL; +
1 -- TYPE OF.

Значение 1 используется, если параметр объявлен как `TYPE OF <domain>` или `TYPE OF COLUMN <table>.<column>`, в остальных случаях используется значение 0.
...
0 -- определённая пользователем; +
1 -- определённая системой.
...
0 -- LEGACY (не используется); +
1 -- SELECTABLE -- селективная хранимая процедура (содержит в своём составе оператор `SUSPEND`); +
2 -- EXECUTABLE -- выполняемая хранимая процедура.
...
|RDB$FORMAT
|SMALLINT
|Текущий номер формата. Для внутреннего использования. Указывает на запись в `RDB$FORMATS`.
